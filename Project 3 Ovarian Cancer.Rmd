...
# multiplot(plotex, plotcn) * warum geht es nicht?
...
```{r}
boxplotex <- qplot(y=exp1$value, x= 1, geom = "boxplot") # create boxplot
+ ggtitle("Gene expression") + xlab("") + ylab("expression") # name both axis
+ theme(axis.text.x=element_blank()) # remove the scaling of the x-axis

boxplotcn <- qplot(y=cn1$value, x= 1, geom = "boxplot") # create boxplot
+ ggtitle("Gene expression") + xlab("") + ylab("expression") # name both axis
+ theme(axis.text.x=element_blank()) # remove the scaling of the x-axis
...
+ geom_point(color = "navy blue") # color
...
```{r}
regression.all.genes <- regression.all.genes[ -sample(1:nrow(regression.all.genes), 200),] # remove row 1-20 
```
...
```{r}
regression.driver <- as.data.frame(t(regression.driver)) # transpose 
```
