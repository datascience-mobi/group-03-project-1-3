...
```
The dataframes, which contains the gene names as rows are ordered alphabetically. This step will later be usefull to compare the different variables for a certain gene.
```{r}
...
In this step the correlation between the gene expression and copy number is tested, to decide wether a linear regression model can be applied for the problem. First the gene expression values and copy number values are fused a single data frame.
```{r}
