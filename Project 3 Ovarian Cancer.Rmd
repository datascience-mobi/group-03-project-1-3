...
anno <- annotation[, which(colnames(annotation) %in% c("gibtsnet"))]


#to seperate the cell lines in groups with similar Mutations we give every Drivermutation a specific value: If TP53 is True in the cell line it gets a "1000" , if it isnt ther will be a "0"
anno$TP53 <- ifelse(rownames(annotation) %in% dmTP53$DepMap_ID, 1, 0)


#same here, ARID1A gets a "100" for True and a "0" for FALSE
anno$ARID1A <- ifelse(rownames(annotation) %in% dmARID1A$DepMap_ID, 1, 0)

#and the same for the next Mutations
anno$ATM <- ifelse(rownames(annotation) %in% dmATM$DepMap_ID, 1, 0)
anno$PTPRF <- ifelse(rownames(annotation) %in% dmPTPRF$DepMap_ID, 1, 0)
anno$SYNE1 <- ifelse(rownames(annotation) %in% dmSYNE1$DepMap_ID, 1, 0)
anno$BAI1 <- ifelse(rownames(annotation) %in% dmBAI1$DepMap_ID, 1, 0)
anno$THBS3 <- ifelse(rownames(annotation) %in% dmTHBS3$DepMap_ID, 1, 0)
...
anno <- anno[, -which(colnames(anno) == "order")]
