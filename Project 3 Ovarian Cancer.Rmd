...
anno_zwei <- data.matrix(anno_zwei)
anno_eins <- anno[, which(colnames(anno) %in% rownames(dm_eins))]
anno_eins <- data.matrix(anno_eins)
...
#and order the matrix after the group
kem <- kem[order(kem$`km[["cluster"]]`, decreasing = TRUE),]
...
apply(kp, 1, function(x) {
  
  #at first the variable has to be loaded with the Drivermutaions that occur three or more times in the cell lines
  kp <- anno_zwei[which(rownames(anno_zwei) %in% kem[which(kem$kem %in% x),]$id),];
  
  kp <- data.matrix(kp)
  #then a heatmap is plotted containing the cell lines of the group with its Drivermutations
  heatmap(kp, Rowv = NA,   main = x ,xlab = "Mutated Gene", ylab = "cell line", col = heat.colors(2),  cexCol = 0.9,    cexRow = 0.6); 
  
  #we add a legend
  legend("topright", pch = 15, col = heat.colors(2), legend = c("not mutated", "mutated"), bty = 'n', cex = 1); 
  
  #for the computer analysis every Drivermutation that occurs two or more times is used
  kp <- anno_eins[which(rownames(anno_eins) %in% kem[which(kem$kem %in% x),]$id),];
...
Hier fehlt noch die Interpretation der PCA bzw der Cluster.
