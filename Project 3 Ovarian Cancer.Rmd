...
## 4. Co-existing Mutations
After identification of the most frequent mutations and the verification of the driver mutations with the literature, this step will aim to detect whether most cell lines only obtain one driver mutation or show a coexistence of several driver mutations.
If mutations often occur together they are possibly linked to each other in their genesis.
First a new object is defined with one column for the mutated genes and another column for the frequency of the specific mutation. 
For the analysis duplicates of muatation in one cell line are not considered. 
```{r}
...
```
To later determine whether a specific mutation is present in a cell line a new object is created. The cells of the new object consistis of the two values one or zero. One indicates that the listed mutation occurs in the cell line, while zero denies the present of the mutation for the cell line. The single genes will be added as columns.
```{r }
# wirklich wichtig? noch umbenennen
dm <- new_uniq[which(new_uniq$Hugo_Symbol %in% dm_null$Var1), ]

# Apply function to annotation data, where one is added in present of the mutation and zero if mutation is not present
...
```
The new object can now be used to calculated the number of mutations for one cell line based on the sum of the single. 
```{r}
# Sum up the values for each cell line to determine the totality of mutated genes
anno$summe <- apply(anno, 1, function(x) { sum(x)})

# Show the characteristics for the column summe 
summary(anno$summe)
```
With a minimal number of 23 mutations for one cell line it is indicated, that mutations do not occur single in cancer cell. On average a cell line exhibits a totality of 89 mutated genes, but it has to be concidered that in this step each mutation is counted. Mutations that only occur in one cell line are included, while they can not be considered as driver mutations. Next only mutated genes that are present in more than three cell lines are used for futher analysis.
````{r}
# Create object with mutations that occur at least in 2 or 4 cell lines (dm_eins is requiered for step 5)
dm_eins <- dm_null[which(dm_null$Freq > 1), ] 
dm_drei <- dm_null[which(dm_null$Freq > 3), ]

# Use the above created objects to remove genes that do not occur at least two or four times (anno_eins is requiered for step 5)
anno_eins <- as.data.frame(anno[, which(colnames(anno) %in% rownames(dm_eins))])
anno <- anno[, which(colnames(anno) %in% rownames(dm_drei))]
```
After the reduction process the sum of the single mutations for each cell line has to be calculated again
```{r}
# sum up the mutated genes for each cell line 
anno$summe <- apply(anno, 1, function(x) { sum(x)})

# order the cell lines with decreaing number of mutations  
anno <- anno[order(anno$summe, decreasing = TRUE),]

# Show the characteristics for the column summe
summary(anno$summe)
```
Only including mutations, that at least occur in four different cell lines, limits the amount of observed mutation. In comparison approximately three to four frequent mutation are observed for each cell line, while the previous analysis indicate a number of 48 mutated genes as median. Also a cell line, which does not contain one of the defined driver mutations, is indicated.
To later apply the data frame anno to a heat map to obtain a visual overview the column has to be removed and the data frame is converted in a matrix
````{r}
...
```
Next the data is shaved in another object, that can be used to create a heatmap. This heatmap indicates based on the colors wheter a specific gene is mutated in a cell line or not.
```{r}
...
# Plot cell lines and mutation in a heatmap 
ggplot(mat.melted, aes(x = Var1, y = Var2, fill = value)) +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, size = 6, hjust = 1)) + ggtitle("Investigation of Co-existing Mutations") + xlab("Cell lines") + ylab("Mutations")
```

The heatmap once more indicate that cell lines with only a single driver mutation are raw, while most cell lines consists of combination of different mutation. The heatmap also shows the cell line with the highest number of mutation left, while the number is decreasing to the right side. Only cell line ACH-000524 obtain none of the frequent mutations, while ten of 34 cell lines do only have one of the definient driver mutations. These cell lines do not indicate a pattern for a common mutation, where a single variantion in the gene is associated with the development of overian cancer. In the 10 cell lines only the two genes TLN2 and PRSS3 show a frequeny of two. 
Each cancer cell exhibits a high amount of gene variation due to mutation, which could result of genome or chromosomal instability. Studies inticate that 33 to 66 mutation occur in cell lines, which lead to an altered aminoacid sequence.
But many of the genomic aberration do not lead to a different phenotype and can therefore be seen as passenger mutation. They result of the genetic instability due to the occurance of a driver mutation (Alberts et al., 2017). 
Therefore it is likly that the heatmap shows driver and passanger mutations, which would have to be verified in experiments, wether the single mutation tend to change the phenotyp of the cell or not. Still the heatmap shows, that most cell lines contain a combination of as driver mutation altered genes. This circumstance has to be considered in the search for synthetic lethality partners, because the data set does not include cell lines with only include one of the determined driver mutations. 

## 5. Synthetic Lethality Interactions
### 5.1 Identification of SL partners with applied statistics
As mentioned in the introduction, it is our goal to seek out synthetic lethality interaction partners for driver mutations occurring in Ovarian cancer. These SL partners are genes that become essential in cancer cells due to the presence of mutations that lead to altered protein function and signaling pathways. The main idea behind identifying the SL partners is finding a way to target cancer cells without affecting healthy cells (O'Neil *et al.*, 2017). A cancer drug leading to effective knockdown of SL partner genes in a patient's tumour could specifically induce cell death in cancer cells. This kind of approach in combination with other therapies would hopefully increase treatment efficacy and reduce unwanted side effects, thus improving life quality.

The task is to find possible synthetic lethality interaction partners for the most frequent driver mutations, these being TP53 and ARID1A. For every driver mutation, our batch of cell lines will be devided into two groups depending on presence or non-presence of the respective driver mutation. 

Using the matrix kd.prob, a one-sided unpaired t-test will be performed in order to calculate the p values for gene essentiality in both groups respectively. It was hereby assumed that SL partners exhibit higher essentiality values in cell lines that possess the respective driver mutation. By extracting the genes that show the lowest p values thus indicating highly significant differences concerning essentiality, potential SL interaction partners can be identified.
First, the SL interaction partner search will be performed for ARID1A:
...
```
In the literature, the gene BRD2 is mentioned as a synthetic lethality interaction partner for ARID1A in Ovarian clear cell carcinomas (Berns *et al.*, 2018). It remains to be found out which significance the difference in survival probability this SL partner displays.
```{r}
# Chech p value for alleged SL partner BRD2
p.values[grep("\\bBRD2\\b", rownames(as.matrix(p.values)))]

```
The p value for BRD2 is 0.1763, which statistically is far away from being significant. It can be concluded that in Ovarian Cancer in general, BRD2 is not a significant SL partner in cell lines with ARID1A mutations. However, the most likely reason for this discrepancy is that Berns *et al.* focused on the Ovarian cancer subtype clear cell carcinoma.

The same statistical testing procedure as was performed for ARID1A will be applied to TP53:
```{r}
...
# Perform SL interaction partner search for TP53
...
To investigate the specific interaction with possible synthetic lethality via PCA the cell lines has to be grouped based on their mutational signatures first.
In the following each of the four driver mutations will receive a different decimal power, which are listed in the table below.
...
Before using the decimal power as sign for the present driver mutations in the cell, a new object is created. In the rows the 34 cell lines are listed and a column for each mutation indicates the combination of mutations for the specific cell line. Each cell with a value of zero show, that the driver mutation (column name) is not present in the cell line, while one a the four different decimal power indicates the existence of the driver mutation. 
...
```
The sum of all the decimal power for each cell line lead to the specific combination of driver mutation, can be used to determine which specific mutations occur in the cell line.
The column summe is now used to name the mutation sequence and will highlight the different groups in the following Principal Component Analysis.A new column for written mutation combinations a is added to the object annodm.
...
To investigate possible SLI partners a Principle Component Analysis (PCA) is performed. In general a PCA can be applied for dimension reduction, reduce a large data set into Pinciple Components, which condense the essential information of the data set. Differences in the data set exhibit the most information, because the data points can be categorized by variables that shows a variance in the data set. As describe in the paragraphs above genes with a high variance are likely to be possible SLI partners.
First the knockdown data is reduced to genes that indactes a variance higher than the 75% quantile.
```{r}
# Calculate variance over all rows for the gene knockdown (probability)
topVar = apply(kd.prob, 1, var) 

# Reduce data set to genes, that indicates a variance higher that the 75% quantile
kd.prob.topVar = kd.prob[topVar > quantile(topVar, probs = 0.75), ] 

# Transpose the data frame
kd.prob.topVar <- t(kd.prob.topVar)
```
Next the reduced knockdown data is applied to a PCA. To obtain a clustering of the different driver mutation combination, the definied groups of the previos step are colored.
```{r}
# Calculate the variance and Principle Components for the knock.down data
PC<-prcomp(kd.prob.topVar)
PCi<-data.frame(PC$x)

# Use different colors for the defined groups in step 5.+++++++++
Mutation <- annodm$kat

# Plot the PCA for and show defined groups in different colors
ggplot(PCi,aes(x=PC1,y=PC2,col=Mutation))+geom_point(size=2.5) + ggtitle("Cell lines clustered by driver mutations")
```
As can be seen, no clustering of the groups, which are defined by their driver mutation pattern, is visible. In this form, the PCA does not support the significant SL partners found by means of the statistical test. The reason for this could be that the observation of many driver mutations at once in a cell line poses a too great complexity of genetic characteristics and can therefore not be plotted this simply. A more direct way to find SLI partners would be to establish cell lines with one driver mutation only and to subsequently introduce further mutations to examine the complex interplay of several mutations.


- no clustering of defined groups
- all seenms to cluster same, some outliers --> but other points of the group does not tent to cluster with outliers
- not useful to determine the load of the PC --> no clear structure based on the driver mutation
- may problem --> driver mut are TSG, if SLIP inhibit protein (activated due to TSG inactivation) other TSG still inactiv, may buffer effect of SLIP
- no hard synthetic lethality interaction partners due to regulatory buffering mechanisms (carl text)
- identification of false driver mutation
- wrong clustering
- To obtain better result --> cell line only contain one specific driver mutation, look for SLIP and afterwards induce other mutations, lead to buffering mechasims?
- easier for oncogenes to inactivat them, instead of inactivate proteins, that a active due to the lack of the TSG-inactivation
