...
The task is to find possible synthetic lethality interaction partners for the most frequent driver mutations, these being TP53 and ARID1A. For every driver mutation, our batch of cell lines will be devided into two groups depending on presence or non-presence of the respective driver mutation. Using the matrix kd.prob, the mean gene essentiality can be calculated for both groups respectively. It was hereby assumed that SL partners exhibit higher essentiality values in cell lines that possess the respective driver mutation. By calculating the difference between the mean essentiality regarding presence or non-presence of a driver mutation and identifying the maximum differences, possible SL partners can be identified. Since theoretically, the number of cell lines in each group is by far not sufficient for the obtained values to be highly significant, a statistical test will be performed in order to validate the results.
