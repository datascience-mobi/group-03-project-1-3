...
#to seperate the cell lines in groups with similar Mutations we give every Drivermutation a specific value: If TP53 is True in the cell line it gets a "1000" , if it isnt ther will be a "0"
anno$has_TP53 <- ifelse(rownames(annotation) %in% dmTP53$DepMap_ID, 1, 0)


#same here, ARID1A gets a "100" for True and a "0" for FALSE
anno$has_ARID1A <- ifelse(rownames(annotation) %in% dmARID1A$DepMap_ID, 1, 0)

#and the same for the next Mutations
anno$has_ATM <- ifelse(rownames(annotation) %in% dmATM$DepMap_ID, 1, 0)
anno$has_PTPRF <- ifelse(rownames(annotation) %in% dmPTPRF$DepMap_ID, 1, 0)
anno$has_BAI1 <- ifelse(rownames(annotation) %in% dmBAI1$DepMap_ID, 1, 0)
anno$has_SYNE1 <- ifelse(rownames(annotation) %in% dmSYNE1$DepMap_ID, 1, 0)
anno$has_THBS3 <- ifelse(rownames(annotation) %in% dmTHBS3$DepMap_ID, 1, 0)

rownames(anno) <- c(1:34)

anno <- anno[order(anno$has_TP53, decreasing = TRUE),]
...
heatmap(anno, Rowv = NA, col = heat.colors(2))
