...
anno1 <- anno
anno1$has_TP53 <- ifelse(rownames(anno) %in% dmTP53$DepMap_ID, TRUE, FALSE)
anno1$has_ARID1A <- ifelse(rownames(anno) %in% dmARID1A$DepMap_ID, TRUE, FALSE)
anno1$has_ATM <- ifelse(rownames(anno) %in% dmATM$DepMap_ID, TRUE, FALSE)
anno1$has_BAI1 <- ifelse(rownames(anno) %in% dmBAI1$DepMap_ID, TRUE, FALSE)
anno1$has_PTPRF <- ifelse(rownames(anno) %in% dmPTPRF$DepMap_ID, TRUE, FALSE)
anno1$has_SYNE1 <- ifelse(rownames(anno) %in% dmSYNE1$DepMap_ID, TRUE, FALSE)
anno1$has_THBS3 <- ifelse(rownames(anno) %in% dmTHBS3$DepMap_ID, TRUE, FALSE)

topVar = apply(kd.prob, 1, var)
tree = kd.prob[topVar > quantile(topVar, probs = 0.75), ]

#tree <- kd.prob[which(rownames(kd.prob) %in% DriverMutation$Var1), ]
...
annodm <- anno[, -which(colnames(anno) %in% c("CCLE_Name", "Subtype.Disease", "has_BAI1", "has_SYNE1", "has_THBS3"))]
...
pcb <- ifelse(annodm$summe == 1000, "blue", ifelse(annodm$summe > 1099, "purple", ifelse(annodm$summe == 100, "pink",  ifelse(annodm$summe == 101, "red", ifelse(annodm$summe < 100, "grey", ifelse(annodm$col == 0, "darkblue", "white"))))))
pcb <- cbind(`TP` = pcb)

annodm$col <- pcb

#alle Gene aus kd.prob

topVar = apply(kd.prob, 1, var)
prob.topVar = kd.prob[topVar > quantile(topVar, probs = 0.75), ]
pca = prcomp(prob.topVar, center = F, scale. = F)
...
pcamat <- kd.prob[which(rownames(kd.prob) %in% DriverMutation$Var1), ]

topVar = apply(pcamat, 1, var)
prob.topVar = pcamat[topVar > quantile(topVar, probs = 0.75), ]
pca = prcomp(prob.topVar, center = F, scale. = F)
plot(pca$rotation[, 1], pca$rotation[, 2], col = pcb[, 1], pch = 19, xlab = "PC1",      ylab = "PC2")
...
pcamat <- kd.ceres[which(rownames(kd.ceres) %in% DriverMutation$Var1), ]
...
pcamat <- kd.prob[which(rownames(kd.prob) %in% DriverMutation1$Var1), ]

topVar = apply(pcamat, 1, var)
prob.topVar = pcamat[topVar > quantile(topVar, probs = 0.65), ]
pca = prcomp(prob.topVar, center = F, scale. = F)
plot(pca$rotation[, 1], pca$rotation[, 2], col = pcb[, 1], pch = 19, xlab = "PC1",      ylab = "PC2")

#nur TP53 und ARID1A 
DriverMutation2 <- DriverMutation1
DriverMutation2 <- DriverMutation2[-which(rownames(DriverMutation) == 183),]
DriverMutation2 <- DriverMutation2[-which(rownames(DriverMutation) == 1757),]
pcamat <- kd.prob[which(rownames(kd.prob) %in% DriverMutation2$Var1), ]

topVar = apply(pcamat, 1, var)

pca = prcomp(pcamat, center = F, scale. = F)
plot(pca$rotation[, 1], pca$rotation[, 2], col = pcb[, 1], pch = 19, xlab = "PC1",      ylab = "PC2")

#nur TP53 und ARID mit ceres
DriverMutation2 <- DriverMutation1
DriverMutation2 <- DriverMutation2[-which(rownames(DriverMutation) == 183),]
DriverMutation2 <- DriverMutation2[-which(rownames(DriverMutation) == 1757),]
pcamat <- kd.ceres[which(rownames(kd.ceres) %in% DriverMutation2$Var1), ]
topVar = apply(pcamat, 1, var)
pca = prcomp(pcamat, center = F, scale. = F)
plot(pca$rotation[, 1], pca$rotation[, 2], col = pcb[, 1], pch = 19, xlab = "PC1",      ylab = "PC2")
...
```
Even both dataframes do not indicate a good fit for normality, futher analysis is performed with the data set.
...
colnames(regression.all.genes) <- c("expression", "copy number") # rename the colums
```
To ensure a clean workspace the no longer needed data frames exp1 and cn1 are removed
```{r}
remove(exp1, cn1)
```
A Create a scatter to check the correlation between the two variables and correlation coefficient in the plot is ablined (sieht nicht so gut aus, was kann ich ändern :( )
```{r}
 plot(expression ~ copynumber, data = regression.all.genes, xlab= "expression", ylab = "copynumber", main="Correlation between all genes", col="blue") + abline(lm(expression ~ copynumber, data = regression.all.genes), col="red")
```
