...
#rownames(DriverMutation) <- DriverMutation$Var1


anno <- apply(dm_drei, 1, function(x) {annotation$ff[x]<- ifelse(rownames(annotation) %in% dm[which(dm$Hugo_Symbol %in% x), ]$DepMap_ID, TRUE, FALSE)})
...
anno <- anno[, -which(colnames(anno) %in% c("summe"))]
...
heatmap(anno, Rowv = NA, Colv = NA, main = "occuring Drivermutations",xlab = "Mutated Gene", ylab = "cell line", col = heat.colors(10),  cexCol = 1.1)

legend("topright", pch = 15, col = cols, legend = co$kat, bty = 'n', cex = 1)
```

In the heatmap we can see cell lines with similar Drivermutations


```{r}
summe <- as.data.frame(summe)

summe$kat <- ifelse(summe == 1000, "TP53 only", ifelse(annodm$summe > 1099, "TP53 & ARID1A", ifelse(annodm$summe == 100, "ARID1A only", ifelse(annodm$summe == 110, "ARID1A & sec. Mu", ifelse(annodm$summe == 101, "ARID1A & sec. Mu", ifelse(annodm$summe < 100, "no TP53 & no ARID1A", ifelse(annodm$summe == 0, "no TP53 & no ARID1A", "TP53 & sec. Mu")))))))
