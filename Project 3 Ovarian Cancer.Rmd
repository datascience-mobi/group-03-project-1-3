...
anno$has_TP53 <- ifelse(rownames(anno) %in% dmTP53$DepMap_ID, TRUE, FALSE)
anno$has_ARID1A <- ifelse(rownames(anno) %in% dmARID1A$DepMap_ID, TRUE, FALSE)
anno$has_ATM <- ifelse(rownames(anno) %in% dmATM$DepMap_ID, TRUE, FALSE)
anno$has_BAI1 <- ifelse(rownames(anno) %in% dmBAI1$DepMap_ID, TRUE, FALSE)
anno$has_PTPRF <- ifelse(rownames(anno) %in% dmPTPRF$DepMap_ID, TRUE, FALSE)
anno$has_SYNE1 <- ifelse(rownames(anno) %in% dmSYNE1$DepMap_ID, TRUE, FALSE)
anno$has_THBS3 <- ifelse(rownames(anno) %in% dmTHBS3$DepMap_ID, TRUE, FALSE)


cb1 <- ifelse(anno$has_TP53 == "TRUE", "forestgreen", "firebrick")
cb <- cbind(`TP53` = cb1)
plot(cor.hc, las = 2, cex.lab = 0.7)
...
plot(pca$rotation[, 1], pca$rotation[, 2], col = cb[, 1], pch = 19, xlab = "PC1",      ylab = "PC2")



annodm <- anno[, -which(colnames(anno) %in% c("CCLE_Name", "Subtype.Disease", "TP53.none", "has_BAI1", "has_SYNE1", "has_THBS3"))]
