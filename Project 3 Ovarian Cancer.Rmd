...
Fuse the knock down data to a single dataframe
```{r}
knock.down <- as.data.frame(rbind.data.frame(kd.ceres, kd.prob))
```
...
knock.down <- knock.down[order(rownames(knock.down)),]
```
...
```{r}
list_all.genes <- list(expression.lr,copynumber.lr, knock.down ) # bind all the dataframes in one list
...
```
To check if the process was succesful, we compared the dimensions of the dataframes before and after the clean up
