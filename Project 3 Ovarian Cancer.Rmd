...
#adding a legend
legend("topright", pch = 15, col = cols, legend = co$kat, bty = 'n', cex = 0.5)
```

In the heatmap it is visible that many drivermutations, especially the ones that occur less than five times in our cell lines, occur very randomly along the cell lines. Caused by this fact we choosed another way to find groups that fit together. To validate our seven main Driver Mutations it was checked if they were mentioned in an Ovarian cancer context in literature. And the ones which were common are: TP53, ARID1A, ATM and PTPRF. In the next step the cell lines are investigated if these Mutations occur in them and we groups are defined:
