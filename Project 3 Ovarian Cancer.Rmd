...
In this step the correlation between the gene expression and copy number is tested, to decide wether a linear regression model can be applied for the problem. First all values for the gene expression and copy number are merged in one dataframe. There for the former established matices exp1 and cn1, but first they have to be replaced after the whole reduction process.
```{r}
...
```
To  analyse the correlation between the gene expression and the copynumber the person and spearman correlation coefficient is computed.
```{r}
