...
anno <- apply(dm_null, 1, function(x) {annotation$ff[x]<- ifelse(rownames(annotation) %in% dm[which(dm$Hugo_Symbol %in% x), ]$DepMap_ID, TRUE, FALSE)})
...
heatmap(anno, Rowv = NA, Colv = NA, main = "occuring Drivermutations",xlab = "Mutated Gene", ylab = "cell line", col = heat.colors(10),  cexCol = 0.9)
...
#we defined another cluster in the step before which was created about he heatmap, we can plot the same PCA with different colors 
