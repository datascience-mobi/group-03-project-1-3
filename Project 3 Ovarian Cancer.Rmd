...
annotation$THBS3 <- ifelse(rownames(annotation) %in% dmTHBS3$DepMap_ID, TRUE, FALSE)
...
#to keep the overview which name was transformed to which number we add the numbers to the annotation matrix in the column "transformed_Numbers""
annotation$transformed_Numbers <- rownames(anno)
...
We take our 7 Driver mutations (TP53, ARID1A, ATM, BAI1, PTPRF, SYNE1, THBS3) and divide the matrix kd.prob into two submatrices: the first one (yesDM group) contains all cell lines with Driver mutations (23 out of 34), the second one contains the remaining cell lines (11 out of 34). Next, similar "cellular behaviour" among the yesDM group is sought out: First, all genes with high variances are sorted out, since we are looking for SL partners that generally display high essentiality). Then, all genes with high mean survival are excluded since we are interested in genes whose knockout leads to cell death (high essentiality).
However, synthetic lethality interactions are defined by being only lethal in cells that have a certain mutation. Therefore, the same procedure described above is also applied to the non-dm group (no driver mutations -> nonDM). Then, it is checked whether there are any genes that do not lead to cell death in both groups, but solely in the dm group - this would be a SL interaction partner.
This method in general can also be specifically applied to other groups, e.g. all cell lines containing ARID1A mutation vs. cell lines without ARID1A mutation.
...
Other idea to find SL partners: calculate mean gene essentiality probability for both initially defined groups with/without driver mutations (TP53, ARID1A, ATM, BAI1, PTPRF, SYNE1, THBS3), plot them and then ask for significant differences (with statistical test!).
...
diffARID1A = m.ARID1A - m.nonARID1A # calculate difference in essentiality (really?)
diffARIDA1$Mean <- diffARID1A$`apply(yesARID1A, 1, mean)` # wie mach ich das schlauer?
diffARID1A <- diffARID1A[which(diffARID1A$Mean > 0.3), ]
diffARID1A <- diffARID1A[order(diffARID1A, decreasing = TRUE),] # warum bei view auf einmal NA values? bei Marco hats doch auch geklappt
barplot(diffARID1A$Mean, names.arg = diffARID1A$Var1, xlab="SL partner", las = 1, horiz=FALSE, cex.axis = 1,2, col = "lightblue", main = "SL partners for ARID1A mutation", ylab = "difference in essentiality") # funktioniert, aber noch mal gucken ob das wirklich sein kann. Erscheint mit irgendwie viel. Wie mach ich hier, dass unten die Gennamen angezeigt werden?

# f�r andere dm fehlt das noch. muss dann nebeneinander am besten angezeigt werden. Dann diskussion!
